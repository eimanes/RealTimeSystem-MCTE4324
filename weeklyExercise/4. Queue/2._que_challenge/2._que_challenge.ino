...
// Settings
static const uint8_t buf_len = 255;       //size of buffer
static const char command[] = "delay";    //space
static const int delay_que_len = 5;       //size of delay_que
static const int msg_que_len = 5;         //size of msg_que
static const uint8_t blink_max = 100;     //num of blink before msg
...
//clear whole buffer
 ...
 // Loop forever
  while (1) {
    ...
    if (xQueueReceive(msg_que, (void *)&rcv_msg, 0) == pdTRUE) {
    ...
    if (Serial.available() > 0) {
      ...
      if ((c == '\n') || (c == '\r')) {
        ...
        if (memcmp(buf, command, cmd_len) == 0) {
          ...
          if (xQueueSend(delay_que, (void *)&led_delay, 10) != pdTRUE) {
...
void blinkLED(void *parameters) {
  ...
  while (1) {
    ...
    // See if there's a message in the queue (do not block)
    if (xQueueReceive(delay_que, (void *)&led_delay, 0) == pdTRUE) {
...
void setup() {
  ...
  // Create queues
  delay_que = xQueueCreate(delay_que_len, sizeof(int));
  msg_que = xQueueCreate(msg_que_len, sizeof(Message));
